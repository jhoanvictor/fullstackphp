...
class App extends Controller
{
    ...
    public function home()
    {
        ...
        $chart = (new AppInvoice())
            ->find("user_id = :user AND status = :status AND due_at >= DATE(now() - INTERVAL 4 MONTH) GROUP BY YEAR(due_at), MONTH(due_at) ORDER BY YEAR(due_at) ASC, MONTH(due_at) ASC",
        ...
        if ($chart) {
            ...
            $chartData->expense = implode(",",
                array_map("abs", $chartExpense)); //abs -> função nativa transforma em numeros positivos inteiros
    ...
    public function filter(array $data)
    {
        ...
        $start = new \DateTime(date("Y-m-t")); //t representa o ultimo dia do ano
        $end = new \DateTime(date("Y-m-t", strtotime("{$y}-{$m}+1month")));
        $diff = $start->diff($end);

        if (!$diff->invert) {
            $afterMonths = (floor($diff->days / 30)); //floor arredonda o resultado da divisão
    ...
    public function launch(array $data): void
    {
        if (request_limit("applaunch", 20, 60 * 5)) {
            $json["message"] = $this->message->warning("Foi muito rápido {$this->user->first_name}! Por favor, agurade 5 minutos para novos lançamentos.")->render();
            echo json_encode($json);
            return;
        }

        if (!empty($data["enrollments"]) && ($data["enrollments"] < 2 || $data["enrollments"] > 420)) {
            $json["message"] = $this->message->warning("Oops {$this->user->first_name}! O número de parcelas deve ser entre 2 e 420.")->render();
            echo json_encode($json);
            return;
        }

        $data = filter_var_array($data, FILTER_SANITIZE_STRIPPED);
        //Caso a data seja anterior ou igual a data atual, o status vem pago, senão não pago
    ...
    public function support(array $data): void
    {
        ...
        if (request_repeat("message", $data["message"])) {
            $json["message"] = $this->message->info("Já recebemos sua solicitação {$this->user->first_name}. Agradecemos pelo contato e retornaremos em breve.")->render();
            echo json_encode($json);
            return;
        }
        ...
        )->send($this->user->email, "{$this->user->first_name} {$this->user->last_name}");
    ...
    public function invoice(array $data): void
    {
        if (!empty($data["update"])) {
            ...
            if ($data["due_day"] < 1 || $data["due_day"] > $dayOfMonth = date("t", strtotime($invoice->due_at))) {
                $json["message"] = $this->message->warning("O vencimento deve ser entre os dias 1 e {$dayOfMonth} para este mês.")->render();
                echo json_encode($json);
                return;
            }
            ...
            $invoiceOf = (new AppInvoice())->find("user_id =:user AND invoice_of = :of",
                "user={$this->user->id}&of={$data["invoice"]}")->fetch(true);
    ...
    public function profile(?array $data): void
    {
        if (!empty($data["update"])) {
            ...
            if (!empty($_FILES["photo"])) {
                ...
                //Caso exista uma foto, remove a atual para salvar uma nova evitando lotar a memória
                if ($this->user->photo()) {
        ...
        echo $this->view->render("profile", [
            "head" => $head,
            "user" => $this->user,
            "photo" => ($this->user->photo() ? image($this->user->photo, 360, 360) : theme("/assets/images/avatar.jpg",
                CONF_VIEW_APP))
        ]);
    }
